...
class HogwartsDatabaseHelper(context: Context) :
    ...
    fun getNumeroAlumnos(casa: String): Int {
        ...
        val cursor = db.rawQuery("SELECT COUNT(*) FROM alumnos WHERE casa = ?", arrayOf(casa))
        cursor.use {
            if (it.moveToFirst()) {
                return it.getInt(0)
            }
    ...
    fun listaAlumnos(casa: String): List<AlumnoHogwarts> {
            ...
            val db = readableDatabase
            val res = db.query("alumnos", null, "casa = '$casa'", null, null, null, "apellido asc")
            val listado = mutableListOf<AlumnoHogwarts>()

            while (res.moveToNext()) {
                val nombre = res.getString(1)
                val familiaMaggle = FuncionesApoyo.integerToBoolean(res.getInt(2))
                val casa = res.getString(4)
                ...
                val alumno = AlumnoHogwarts(nombre, familiaMaggle, casa, apellido, habilidad, inteligencia, creatividad, etica, coraje, lealtad)
                listado.add(alumno)
            }
